...
<head>
    ...
    <title>Library System - My Borrowed Books</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item"><a class="nav-link" href="#" id="logoutButton">Logout<i class="fa fa-fw fa-sign-out-alt"></i></a></li>
...
<div class="container">
    ...
    @forelse($borrowedBooks as $borrowedBook)
        @php
            $borrowedDate = \Carbon\Carbon::parse($borrowedBook->borrowed_at);
            $dueDate = $borrowedBook->due_date;
            $isOverdue = now()->greaterThan($dueDate);
        @endphp
        <li class="{{ $isOverdue ? 'overdue' : 'on-time' }}">
            <h3>{{ $borrowedBook->book->bookname }}</h3>
            <p>Borrowed Date: {{ $borrowedDate->format('Y-m-d') }}</p>
            <p>Due Date: {{ $dueDate->format('Y-m-d') }}</p>
            <p>Number of Copies Borrowed: {{ $borrowedBook->no_of_copies }}</p>
            <p>Status: 
                @if($isOverdue)
            ...
            <form action="{{ route('User.returnBook', $borrowedBook->id) }}" method="POST">
                @csrf
